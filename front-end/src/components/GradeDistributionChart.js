...
const GradeDistributionChart = ({ 
  ...
  const highlightBorderColor = '#34495e ';
  ...
  const handleMouseEnter = (data, index, fileIndex) => {
    setActiveBar({ index, fileIndex });
  };
        ...
        {gradeDistribution.length > 0 ? (
          <ResponsiveContainer width="100%" height={400}>
            ...
            >
              ...
              />
              <Legend />
              {selectedFiles.map((filename, fileIndex) => (
                <Bar
                  key={filename}
                  dataKey={filename}
                  fill={barColors[fileIndex % barColors.length]}
                ...
                >
                  {gradeDistribution.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={barColors[fileIndex % barColors.length]}
                      stroke={activeBar && activeBar.index === index && activeBar.fileIndex === fileIndex ? highlightBorderColor : 'none'}
                      strokeWidth={1}
                      onMouseEnter={() => handleMouseEnter(entry, index, fileIndex)}
                    />
                  ))}
